...
function Entry({ entry }: { entry: typeof entries[0] }) {
  ...
  return (
    <div ref={ref} className="grid grid-cols-1 md:grid-cols-12 gap-6 md:gap-0 relative">

      {/* Year column — slide from left */}
      <motion.div
        ...
        <div>
          <span className="font-signal leading-none block outlined" style={{ fontSize: 'clamp(4rem, 8vw, 8rem)' }}>
            {entry.year}
          </span>
          {entry.present && (
            <div className="flex items-center gap-1.5 mt-1">
              ...
              <span className="font-rdna text-[0.48rem] tracking-[0.28em] uppercase" style={{ color: 'var(--accent)' }}>
                Present
              </span>
            </div>
          )}
        ...
        <a href={entry.url} target="_blank" rel="noopener noreferrer" className="block group">
          <motion.div className="rounded-sm p-7 md:p-9 relative overflow-hidden"
            ...
            <div className="pl-4">
              <div className="flex items-start justify-between mb-6">
                <div>
                  <div className="flex items-center gap-3 mb-1">
                    <h3 className="font-signal tracking-[0.04em] text-chalk" style={{ fontSize: 'clamp(1.8rem, 3.5vw, 3rem)' }}>
                      {entry.company}
                    </h3>
                    <span className="font-rdna text-[0.48rem] tracking-[0.2em] uppercase px-2.5 py-1 rounded-sm"
                          style={{ color: 'var(--accent)', border: '1px solid rgba(var(--accent-rgb),0.3)', background: 'rgba(var(--accent-rgb),0.07)' }}>
                      Active
                    </span>
                  </div>
                  <p className="font-systems italic text-sm text-white/45">{entry.role} — {entry.type}</p>
              ...
              <p className="font-systems text-sm leading-relaxed text-white/58 mb-5">{entry.summary}</p>
  ...
  return (
    <section ref={ref} id="experience" className="relative py-28 px-6 md:px-12 max-w-7xl mx-auto overflow-hidden">
      <motion.div style={{ y: bgY }}
        ...
        aria-hidden="true">
        <span className="font-signal outlined-chalk opacity-[0.022]" style={{ fontSize: '28vw', lineHeight: 1 }}>
          2024
        </span>
      </motion.div>
      ...
      <div className="relative">
        {entries.map((e) => <Entry key={e.company} entry={e} />)}
      </div>
    </section>
  )
}
