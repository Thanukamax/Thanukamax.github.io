...
const STRENGTH = [60, 36, 16, 6]

export default function Systems() {
  const reduced    = useReducedMotion()
  const sectionRef = useRef<HTMLElement>(null)
  const cloudRef   = useRef<HTMLDivElement>(null)
  const inView     = useInView(sectionRef, { once: false, amount: 0.1, margin: '-10%' })
  ...
  useEffect(() => {
    const update = () => { rectRef.current = cloudRef.current?.getBoundingClientRect() ?? null }
    update()
  ...
  const onMouseMove = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    if (now - lastMoveMs.current < 16.67) return
  ...
  return (
    <section ref={sectionRef} id="systems" className="relative py-24 px-6 md:px-12 overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto">
          ...
          {CLOUD.map((skill, i) => {
            ...
            return (
              <motion.div
                ...
                animate={inView
                  ? (reduced ? { opacity: skill.op } : { opacity: skill.op, scale: 1 })
                  : (reduced ? { opacity: 0 } : { opacity: 0, scale: 0.72 })}
                whileHover={{ opacity: 1, scale: 1.06, transition: { duration: 0.15, ease: 'easeOut' } }}
                transition={{ delay: i * 0.04 + 0.05, duration: 0.55, ease: E }}
              >
                {skill.text}
              </motion.div>
            )
        ...
        {/* Mobile: autonomous floating cloud */}
        <div
        ...
        >
          {CLOUD.map((skill, i) => {
            ...
            const driftX = reduced ? 0 : driftAmp * Math.cos(goldenAngle)
            const driftY = reduced ? 0 : driftAmp * Math.sin(goldenAngle)
            const mobileRem = Math.min(parseFloat(skill.size) * 0.7, 2)
            return (
              <motion.div
                key={skill.text}
                className={`absolute ${skill.font} leading-none whitespace-nowrap cursor-default`}
                style={{ left: `${skill.x}%`, top: `${skill.y}%`, fontSize: `${mobileRem}rem`, color: skill.color }}
                initial={{ opacity: 0 }}
                animate={inView ? {
                  opacity: skill.op,
                  ...(reduced ? {} : {
                ...
                transition={inView ? {
                  opacity: { delay: i * 0.05 + 0.1, duration: 0.7, ease: E },
